...
class ToDoModelClass {
  int? taskNo;
  String? userNo = SignInPageState.getNumber;
  String title;
  String description;
  String date;
  ...
  @override
  String toString() {
    return "{taskNo: $taskNo, title: $title, description: $description, date: $date, userNumber: $userNo}";
  }
}
...
List getCurrentUserData(List cardsData) {
  for (int index = 0; index < cardsData.length; index++) {
    if (cardsData[index]['userNo'] != SignInPageState.getNumber) {
      cardsData.remove([index]);
    }
  }
  return cardsData;
}
...
class _ToDoAppState extends State<ToDoList> {
  ...
  static String getName() {
    for (int i = 0; i < usersDataList.length; i++) {
      if (SignInPageState.getNumber == usersDataList[i].number) {
        return usersDataList[i].name;
      }
    }
    return "";
  }
              ...
              Container(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    print(cardsList);
                    print("My Number: ${SignInPageState.getNumber}");
                    setState(() {});
                  },
                  child: Text(
                    "Submit",
  ...
  void didChangeDependencies() async {
    ...
    cardsList = await getCurrentUserData(cardsData);
